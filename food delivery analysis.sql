...
-- 1. Top 3 outlets by cuisine types without using limit and top function

with cte as(
select Restaurant_id, cuisine , count(*) as no_of_orders from orders
group by Restaurant_id, cuisine), cte2 as(

select Restaurant_id, cuisine , no_of_orders, row_number() over (order by no_of_orders desc) as rnk from cte)

select Restaurant_id, cuisine, rnk from cte2
where rnk <= 3;

-- 2 Find the daily new customer count from launch date ( everyday how many new customers are we acquiring)

with cte as(
select customer_code, cast(min(placed_at) as Date) as day_placed from orders
group by customer_code
)

select day_placed, count(*) as no_of_new_customer from cte
group by day_placed
order by day_placed;
...
select customer_code, count(*) as no_of_customer from orders
group by customer_code
having count(*) = 1 and min(month(placed_at)) = 01 and min(year(placed_at)) = 2025;


-- 4. Growth team is planning to create a trigger that will target customers after 
...
-- 5. list customers who placed more than 1 order and all their orders on a promo only

select customer_code, count(*) as no_of_order , count(promo_code_name)  as promo_code from orders
where customer_code 
group by customer_code
having count(*) > 1 and count(*) = count(promo_code_name); 

-- 6. what percent of customers were originally aquired in jan 2025. (placed their first order without promo code)



with cte as(
select customer_code, order_id, cast(placed_at as date) as date , coalesce(promo_code_name, 'null') as promo_code_name ,
row_number() over (partition by customer_code order by placed_at) as rnk from orders 
where cast(placed_at as date) between '2025-01-01' and '2025-01-31'), cte2 as (

select customer_code, promo_code_name, rnk from cte 
where promo_code_name = 'null' and rnk = 1)

select round(count(customer_code)*100/  (select count(distinct customer_code) from cte),2) as prcnt from cte2; 
